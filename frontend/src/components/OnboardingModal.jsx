    ...
    content: (
      <div className="space-y-3 text-sm text-slate-300 leading-relaxed">
        <p>
          ...
          <span className="text-white font-medium">a fixed supply of 21 million coins — forever.</span>
        </p>
    ...
    content: (
      <div className="space-y-3 text-sm text-slate-300 leading-relaxed">
        ...
        <div className="bg-slate-700/50 rounded-lg p-4 my-3">
          <p className="text-slate-400 text-xs uppercase tracking-wider mb-2">The 4% Rule</p>
          <p className="text-white font-mono text-sm">FIRE Number = Annual Expenses ÷ 0.04</p>
          <p className="text-slate-400 text-xs mt-2">$3,000/month → $900,000 FIRE target</p>
        </div>
        <p>
          A small BTC allocation — even 5–20% of your portfolio — can{' '}
          <span className="text-white font-medium">dramatically shorten your path to freedom</span> through Bitcoin's asymmetric upside.
        </p>
        <p className="text-slate-400 text-xs">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-700">
          <div className="flex gap-1.5">
            {STEPS.map((_, i) => (
              <div
                key={i}
                className={`h-1.5 rounded-full transition-all ${i === step ? 'w-6 bg-orange-400' : 'w-1.5 bg-slate-600'}`}
              />
            ))}
          </div>
