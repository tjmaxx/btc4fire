...
export const useRealtimePrice = (refreshInterval = 60000) => {
  ...
  useEffect(() => {
    ...
    const fetchPrice = async () => {
      try {
        const data = await apiFetch('/btc-data/price');
...
export const useHistoricalPrice = (days = 7) => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        const result = await apiFetch(`/btc-data/history?days=${days}`);
...
export const useTechnicalData = (days = 30) => {
  ...
  useEffect(() => {
    ...
    const fetchTechnical = async () => {
      try {
        const result = await apiFetch(`/btc-data/technical?days=${days}`);
        if (!cancelled) { setData(result); setError(null); }
      } catch (err) {
        if (!cancelled) setError(err.message);
