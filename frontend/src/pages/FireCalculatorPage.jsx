      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">FIRE Calculator</h1>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-300 dark:border-slate-600 text-gray-900 dark:text-white pl-7 pr-4 py-2 rounded-lg text-sm focus:outline-none focus:border-orange-500"
                />
              </div>
            </div>
            <div>
        ...
        {results && (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="bg-gray-100 dark:bg-slate-800 rounded-xl p-5 border border-orange-500/30">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-gray-500 dark:text-slate-400 text-xs font-medium">FIRE Number</p>
                  <Target className="w-4 h-4 text-orange-400" />
                </div>
                <p className="text-gray-900 dark:text-white text-2xl font-bold">{fmt$(results.fireNumber)}</p>
                <p className="text-gray-400 dark:text-slate-500 text-xs mt-0.5">at {withdrawalRate}% withdrawal</p>
              </div>

              <div className="bg-gray-100 dark:bg-slate-800 rounded-xl p-5 border border-gray-200 dark:border-slate-700">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-gray-500 dark:text-slate-400 text-xs font-medium">BTC Needed</p>
                  <Bitcoin className="w-4 h-4 text-orange-400" />
                </div>
                <p className="text-gray-900 dark:text-white text-2xl font-bold">
                  {results.btcNeeded != null ? results.btcNeeded.toFixed(4) : '—'}
                </p>
                <p className="text-gray-400 dark:text-slate-500 text-xs mt-0.5">BTC at current price</p>
              </div>

              <div className="bg-gray-100 dark:bg-slate-800 rounded-xl p-5 border border-gray-200 dark:border-slate-700">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-gray-500 dark:text-slate-400 text-xs font-medium">You Have</p>
                  <DollarSign className="w-4 h-4 text-blue-400" />
                </div>
                <p className="text-gray-900 dark:text-white text-2xl font-bold">
                  {results.btcCurrent != null ? results.btcCurrent.toFixed(4) : '—'}
                </p>
                <p className="text-gray-400 dark:text-slate-500 text-xs mt-0.5">BTC currently</p>
              </div>

              <div className="bg-gray-100 dark:bg-slate-800 rounded-xl p-5 border border-gray-200 dark:border-slate-700">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-gray-500 dark:text-slate-400 text-xs font-medium">Years to FIRE</p>
                  <Flame className="w-4 h-4 text-yellow-400" />
                </div>
                {results.yearsToFire === 0 ? (
                  <p className="text-green-400 text-2xl font-bold">Now! 🎯</p>
                ) : results.yearsToFire != null ? (
                  <p className="text-gray-900 dark:text-white text-2xl font-bold">{results.yearsToFire.toFixed(1)}</p>
                ) : (
                  <p className="text-gray-400 dark:text-slate-500 text-2xl font-bold">—</p>
                )}
                <p className="text-gray-400 dark:text-slate-500 text-xs mt-0.5">at {annualReturn}% annual growth</p>
              </div>
            </div>

            {/* Chart */}
            <div className="bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5 mb-6">
              <h2 className="text-gray-900 dark:text-white font-semibold mb-4">30-Year Projection</h2>
              ...
              <p className="text-gray-300 dark:text-slate-600 text-xs mt-2 text-center">Assumes {annualReturn}% annual BTC return compounded yearly. Not financial advice.</p>
            </div>
            ...
            <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-5 mb-6">
              <p className="text-orange-600 dark:text-orange-300 text-sm leading-relaxed">
                ...
                you need a portfolio of <span className="font-semibold text-gray-900 dark:text-white">{fmt$(results.fireNumber)}</span>.
                {results.btcNeeded != null && (
                  <> At today's price that's{' '}
                    <span className="font-semibold text-gray-900 dark:text-white">{results.btcNeeded.toFixed(4)} BTC</span>
        ...
        <div className="bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5 text-center">
          <p className="text-gray-700 dark:text-slate-300 mb-3">Start tracking your Bitcoin stack toward your FIRE goal.</p>
          <Link
            to="/portfolio"
