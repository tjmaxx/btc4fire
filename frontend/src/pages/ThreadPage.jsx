  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div>
          <div className="text-white text-sm font-medium">{post.profiles?.username || 'Anonymous'}</div>
  ...
  return (
    <Layout>
      <div className="max-w-3xl mx-auto">
        ...
        {/* Original post */}
        <div className="bg-slate-800 border border-orange-500/30 rounded-2xl p-5 mb-6">
          <div className="flex items-center gap-2 mb-3">
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${CAT_BADGE[thread.category] || 'bg-slate-700 text-slate-400'}`}>
              {thread.category}
            </span>
          ...
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-slate-300 text-sm font-medium">{thread.profiles?.username || 'Anonymous'}</span>
          </div>

          <p className="text-slate-300 leading-relaxed whitespace-pre-wrap">{thread.content}</p>

          <div className="mt-4 pt-4 border-t border-slate-700 flex items-center gap-4 text-slate-500 text-xs">
        ...
        {!thread.locked ? (
          isAuthenticated ? (
            <form onSubmit={handleReply} className="bg-slate-800 border border-slate-700 rounded-xl p-4">
              <h3 className="text-white font-medium mb-3">Add a Reply</h3>
              <textarea
                value={reply}
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 text-center">
              <p className="text-slate-300 mb-3">Sign in to join the discussion</p>
